...
*   **動態調整**：支援根據餘額自動計算下單量。
*   **AdTrack 特化邏輯 (已實作)**：
    *   **4 級階梯止盈**：當訊號產生時，自動拆分 4 份 (各 25%) 的限價平倉單。
    *   **動態移動止損 (Trailing SL)**：
        *   TP1 成交 -> 止損移至進場本金位。
        *   TP2 成交 -> 止損移至 TP1 位置。
        *   以此類推，實現保護利潤與鎖定盈餘。
...
### 6.1 Git 常用維護指令
*   **日常保存**：當開發完成一個階段或功能時，執行以下操作：
    ```bash
    git add .
    git commit -m "描述你的改動 (例如: feat: 新增 Telegram 監聽)"
    git push
    ```
*   **同步更新**：在多台設備間同步最新代碼：
    ```bash
...
### 6.2 依賴與設定管理
*   **更新依賴庫**：當使用 `pip install` 新增了任何 Python 套件，請務必更新：
    ```bash
    pip freeze > requirements.txt
    ```
*   **配置範本更新**：若在 `config.yaml` 增加了新的配置項目，請同步更新同步更新 `config.yaml.example` 並隱藏具體數值。
*   **敏感資訊檢查**：上傳 GitHub 前，請務必確認 API Key 與私鑰未出現在 Git 追蹤範圍內（檢查 `.gitignore`）。